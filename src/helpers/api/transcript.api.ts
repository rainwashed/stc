...
let fetchRoute =
    "https://studentconnect.bloomfield.org/studentportal/Home/LoadProfileData/Transcript";
let labelFilteringCriteria = /( )/gm;
let valueFilteringCriteria = /(&nbsp;|"|'|)/gm;

async function requestTranscriptData(sessionToken: string) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // TODO: Work on the transcript api (my account doesn't have any data)
        } catch (error: any) {
            if (error.response) {
                reject(
...
export default requestTranscriptData;
