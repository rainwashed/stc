...
let fetchRoute =
    "https://studentconnect.bloomfield.org/studentportal/Home/LoadProfileData/Standards";
let labelFilteringCriteria = /( )/gm;
let valueFilteringCriteria = /(&nbsp;|"|'|)/gm;

async function requestStandardsData(sessionToken: string) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // todo: do when there is data in the system
        } catch (error: any) {
            if (error.response) {
                reject(
...
export default requestStandardsData;
